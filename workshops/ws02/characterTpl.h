...
namespace seneca {
    ...
    public:
        ...
        void takeDamage(int dmg) override {
            m_health -= static_cast<T>(dmg); // Subtract damage from current health

            // Check if the character is defeated
            if (getHealth() <= 0) {
                std::cout << getName() << " has been defeated!" << std::endl;
                m_health = static_cast<T>(0); // Ensure health doesn't go negative
            } else {
        ...
        // Example attack amount for the character
        int getAttackAmnt() const override {
            return 10; // Example attack amount
        }

        // Example defense amount for the character
        int getDefenseAmnt() const override {
            return 5; // Example defense amount
        }

        // Cloning method for the character
